function [sulphurConst] = sulphurConstraints_final_final(model,bnds,condition,sulphurCount,cofPairs,minmax,runFinalMinMax,limEX,totalsulphur,skipRxns,direction)
[~,num_rxns] = size(model.S);

%% This script is designed to constrain the solution space based on the 
%   number of sulphur atoms made available to the model by the exchange
...
%
%   sulphurCount =   sulphurCount Structure as received form function 
%                   findMetsulphur.m (needs to contain sulphurCount.sulphurAtoms field)
%
...
%                   supposed to be a single sulphur constraint (e.g. left
%                   over sulphur can be set to CO2 rate which is a single
%                   sulphur. If flux for CO2 (closing the sulphur in excel)
%                   is 0.641 mmol/gDCW*h this would be the rate to fed into
%                   the limEX variable).
%
% OUTPUT
%   sulphurConst =   structure containing the following fields
%                       - Title
%                       - Date and Time of job
%                       - constraintsName (given: e.g. 'LacProd')
%                       - model.description
%                       - total sulphur consumed
%               *** optional ***
%                       - actual sulphur used for constraining (only when
...
%                           - metName
%                           - metFormula
%                           - sulphur atoms for each metabolite
...
%                       - newMinMax pased on sulphur balance
...
%                       - limEX (single sulphur limit passed into fucntion
...
% retrieve the actual metabolite IDs for sulphur calculations
...
%% check if number of sulphur atoms per metabolite was passed into function
if exist('sulphurCount','var')
    if isempty(sulphurCount)
        sulphurCount = findMetCarbon(model);
    end
else
    sulphurCount = findMetCarbon(model);
end
% extract the sulphur vector (length of model.mets)
metCarb = sulphurCount.sulfurAtoms;
...
%% multiply the lower and upper bnds witht their respective number of sulphur atoms
varMinCarb = varMin.*metCarb(metID);
if exist('totalsulphur','var')
    if isempty(totalsulphur)
        total_sulphur_cons = sum(abs(varMinCarb(varMinCarb<0)));
    elseif ~isnumeric(totalsulphur)
        h = warndlg('Warning the totalsulphur input is not numeric. The totalsulphur will be calculated based on the lower bnds of the input variables. If you wish to continue press OK or else kill the code and input a numeric value for totalsulphur.','Problem with input');
        total_sulphur_cons = sum(abs(varMinCarb(varMinCarb<0)));
    else
        total_sulphur_cons = totalsulphur;
        actual_total_sulphur_cons = sum(abs(varMinCarb(varMinCarb<0)));
    end
else
    total_sulphur_cons = sum(abs(varMinCarb(varMinCarb<0)));
end
total_sulphur_cons = total_sulphur_cons;
% if ismember(condition,'exponential')
%     total_sulphur_cons = total_sulphur_cons * 0.05;
% elseif ismember(condition,'stationary')
%     total_sulphur_cons = total_sulphur_cons * 0.01;
% end
...
for i = 1:num_rxns
    ...
    if (i == findRxnIDs(model,'BIOMASS_cho')) % Biomass non producing
        ...
        % Calculating the sulphur same as b4
        subsulphur = sum(abs(full(model.S(subIDs,i))).*metCarb(subIDs,1));
        bericht{i,1} ='Biomass nonproducing';
        constraint = abs(total_sulphur_cons/subsulphur);
    ...
    elseif (i == findRxnIDs(model,'igg_lc')) %LC Synthesis
        ...
        % Calculating the sulphur same as b4
        subsulphur = sum(abs(full(model.S(subIDs,i))).*metCarb(subIDs,1));
        bericht{i,1} ='IgG Light Chain Synthesis';
        constraint = abs(total_sulphur_cons/subsulphur);
        if abs(minmax(i,2)) > constraint
            minmax_new(i,2) = constraint;
            mmAdj(i,2) = 1;  
    ...
    if ismember(i,varID_tmp)
        % If rxn is measured, then we don't set sulphur constraints
    ...
    % evaluate if rxn is balanced or unbalanced
        ...
        elseif ismember(i,isDrain)    
            ...
            % account for stoichiometric coefficients and add up sulphur
            % As we set all the lower bounds for the exchange reactions to
            % 0, the atom constrain will take effect only on the upper
            % bound
            subsulphur = sum(abs(full(model_tmp.S(subIDs,i))).*metCarb(subIDs,1));
            prodsulphur = sum(abs(full(model_tmp.S(prodIDs,i))).*metCarb(prodIDs,1));

            if subsulphur == 0 && prodsulphur == 0 
                continue
            elseif subsulphur == 0
                constraint = abs(total_sulphur_cons/prodsulphur);
            else
                constraint = abs(total_sulphur_cons/subsulphur);
            end
            ...
            if ismember(i,coa.coaRxnIDs)
                if coa.missMatch
                    warning('Stoichiometry of CoA is not matching. Smaller number is subtracted from sulphur balance. Check reaction reaction manually.')
                    if coa.subAtom(ismember(coa.coaRxnIDs,i)) > coa.subsulphur(ismember(coa.coaRxnIDs,i))
                        subsulphur = subsulphur - coa.subAtom(ismember(coa.coaRxnIDs,i));
                        prodsulphur = prodsulphur - coa.prdAtom(ismember(coa.coaRxnIDs,i));
                    else
                        subsulphur = subsulphur - coa.subAtom(ismember(coa.coaRxnIDs,i));
                        prodsulphur = prodsulphur - coa.prdAtom(ismember(coa.coaRxnIDs,i));
                    end
            ...
            if ismember(i,lrgMlc.lmRxnIDs)
                subsulphur = subsulphur - (lrgMlc.sub(ismember(lrgMlc.lmRxnIDs,i)));
                prodsulphur = prodsulphur - lrgMlc.prd(ismember(lrgMlc.lmRxnIDs,i));
            end         
            
            % Check whether the number of atoms in the substrates or
            % products are equal to 0; even if the minmax is different tha
            % 0 the molecules could not have any sulphurs or nitrogens etc.                 
            if subsulphur == 0 && prodsulphur == 0
                bericht{i,1} = 'no S in the reaction';
                continue
            end

            % If in fact, the reaction is balanced then do the following:
            if subsulphur == prodsulphur     
                ...
                if find(ismember(cofMetIDs,find(model.S(:,i))))
                    % if the sum of all sulphur is even then it is balanced 
                    % (not bulletproof but if off by only one sulphur it works)
                    % atoms for the substrates, products and the coenzymes
                    % - cofactors
                    if mod(sum([subsulphur;prodsulphur;metCarb(cofMetIDs(ismember(cofMetIDs,find(model.S(:,i)))))]),2) == 0 
                        ...
                        % is a cofactor) so the sulphur of atp/amp should not be considered when constraining
                        if subsulphur > prodsulphur 
                            constraint = abs(total_sulphur_cons/prodsulphur);
                        else
                            constraint = abs(total_sulphur_cons/subsulphur);
...
sulphurConst.title = '*** sulphur closure constraining ***';
sulphurConst.dateAndTime = datetime();
sulphurConst.constraintsName = condition;
if isfield(model,'description')
    sulphurConst.model = model.description;
end
sulphurConst.totalsulphurConsumed = total_sulphur_cons;
if exist('actual_total_sulphur_cons','var')
    if ~isempty(actual_total_sulphur_cons)
        sulphurConst.actualTotalsulphurConsumed = actual_total_sulphur_cons;
    end
end
sulphurConst.report.summary = bericht;
sulphurConst.report.unbalanced = unbRxns;
sulphurConst.report.balanced = balRxns;
sulphurConst.report.setRxns = setRxns;
sulphurConst.report.noFlux = nfRxns;
sulphurConst.report.Unknowns = Unknowns;
sulphurConst.bndAjusted.rawData = mmAdj;
sulphurConst.bndAjusted.numberOf_LB_adj = sum(mmAdj(:,1));
sulphurConst.bndAjusted.numberOf_UB_adj = sum(mmAdj(:,2));
sulphurConst.CoAevaluation = coa;
sulphurConst.sulphurCount = sulphurCount;
sulphurConst.cofactorPairs = cofPairs;
sulphurConst.originalMinMax = minmax;
sulphurConst.newMinMax = minmax_new;
sulphurConst.solSpaceReduction = strcat(num2str(diff),'%');
...
sulphurConst.solutionNew = sol;

%% constrain uptakes based on passed in limit (e.g. lost/closed sulphur)

if exist('limEX','var')
    if ~isempty(limEX)
        sulphurConst.EXrxnAdjust = '*** EX rxns adjustment ***';
        sulphurConst.limEX = limEX;
        ...
        % get metIDs from EX rxn IDs so sulphur per met can be determined
        ...
        if ~isempty(adjEX)
            ...
            sulphurConst.numAdjusted = size(adjEX,1);
            sulphurConst.solSpaceReductionEX  = strcat(num2str(diff_n),'%');
            sulphurConst.EXspaceReduction = strcat(num2str(diffLim),'%');
        else
            warning('no adjustment of EXrxns was done as already constrained more than limEX')
            sulphurConst.warning = 'no adjustment was done';
        end
    end
end
...
if runFinalMinMax
    ...
    sulphurConst.postNmm = '*** post new minmax ***';
    sulphurConst.postNewMinMax = minmax_post_new;
    % qunatify how much solution space was constraint compared to original minmax
    mm_post_diff = minmax_post_new(:,2)-minmax_post_new(:,1);
    diff = 100*(1-(sum(mm_post_diff)/sum(mm_diff)));
    sulphurConst.solSpaceReductionPostCC = strcat(num2str(diff),'%');
    ...
    sulphurConst.solutionPostNew = sol;
end
end
